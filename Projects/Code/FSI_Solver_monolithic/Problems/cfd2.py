...
common = {"mesh": mesh_file,
 ...
 #"checkpoint": "./FSI_fresh_checkpoints/CFD-2/P-2/dt-0.05/dvpFile.h5"
vars().update(common)
...
if checkpoint == "Checkpoints/CFD-2/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvpFile.h5":
    sys.exit(0)
else:
    dvp_file=HDF5File(mpi_comm_world(), "Checkpoints/CFD-2/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvpFile.h5", "w")
...
def initiate(P, v_deg, d_deg, p_deg, dt, theta, dvp_, args, Det_list, refi, mesh_file, mesh_name, **semimp_namespace):
    ...
    if args.extravari == "alfa":
        path = "Results/CFD-2/%(exva)s_%(extype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
    if args.extravari == "biharmonic" or args.extravari == "laplace" or args.extravari == "elastic":
        path = "Results/CFD-2/%(exva)s_%(bitype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
...
def post_process(path,T,dt,Det_list,dis_x,dis_y, Drag_list,Lift_list, Time_list,\
                args, simtime,v_deg, p_deg, d_deg, dvp_file,DVP,mesh_file,**semimp_namespace):

...
for coord in mesh.coordinates():
    if coord[0]==0.6 and (0.199<=coord[1]<=0.2001): # to get the point [0.2,0.6] end of bar
        print coord #Prints be sure we have the correct point.
        break
# BOUNDARIES
...
inlet = Inlet(Um,degree=2)


if checkpoint == "Checkpoints/FSI-1/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvpFile.h5":
    sys.exit(0)
else:
    dvp_file=HDF5File(mpi_comm_world(), "Checkpoints/FSI-1/P-"+str(v_deg)+"/dt-"+str(dt)+"/dvpFile.h5", "w")
...
def initiate(v_deg, d_deg, p_deg, dt, theta, dvp_, args, mesh_name, refi, **semimp_namespace):
    ...
    if args.extravari == "alfa":
        path = "Results/FSI-1/%(exva)s_%(extype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
    if args.extravari == "biharmonic" or args.extravari == "laplace" or args.extravari == "elastic":
        path = "Results/FSI-1/%(exva)s_%(bitype)s/dt-%(dt)g_theta-%(theta)g/%(mesh_name)s_refine_%(refi)d_v_deg_%(v_deg)s_d_deg_%(d_deg)s_p_deg_%(p_deg)s" % vars()
...
def create_bcs(DVP, args, dvp_, n, k, Um, H, boundaries, inlet,mesh_file, **semimp_namespace):
    print "Create bcs"
...
def post_process(path,T,dt,Det_list,dis_x,dis_y, Drag_list,Lift_list, Time_list,\
                mesh_name, args, simtime,v_deg, p_deg, d_deg, dvp_file,DVP,mesh_file,**semimp_namespace):
    #dvp_file.close()
